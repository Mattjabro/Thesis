...
FASTQ files through the Divisive Amplicon Denoising Algorithm 2 (DADA2) pipeline (Callahan et al.\ 2016). The second is the Earth Microbiome Project (EMP)
...
distances geometrically inappropriate and Pearson correlations spurious\cite{aitchison1986}\cite{gloor2017}. Soil microbiome data are
...
The centered log-ratio (CLR) transformation \cite{aitchison1986}\cite{gloor2017} maps
...
\begin{figure}[H]
  ...
  \includegraphics[width=1\linewidth]{Fig3_Thesis.png}      
    ...
    Centered log-ratio (CLR) transformation pipeline \cite{aitchison1986}\cite{gloor2017}, applied identically to both datasets. Input: sparse
...
\cite{fierer2006}\cite{lauber2009}. Two soils
...
dataset suggests that there are differences in litter chemistry between biomes. Tropical forests
producing more recalcitrant leaf litter than temperate deciduous forests, for example,
are reflected in the abundance of polysaccharide-degrading Verrucomicrobiota.
...
waterlogged soils\cite{treude2003}\cite{thomas2008}.
...
is moderate. This is not a limitation of the analytical methods; it may be a property of the
biological system. This reflects a biological reality: soil microbial communities across terrestrial biomes share many widespread genera, and differences between biomes arise mainly from shifts in relative abundance rather than entire turnover of taxa.
...
provides taxonomic information only and cannot directly resolve their function. Two
...
This thesis applied a consistent analytical framework: centered log-ratio transformation, mixture of Gaussian Graphical Models clustering, and random forest classification to two independent global soil microbiome datasets derived from 16S rRNA amplicon sequencing. The global topsoil dataset (Bahram et al.\ 2018) provided 193 samples from a single coordinated survey of terrestrial biomes, while the Earth Microbiome Project soil subset (Thompson et al.\ 2017) provided 2,209 samples spanning terrestrial, marine, and freshwater environments across 97 independent studies.
...
supporting the results reported in the main text.
...
Global Topsoil & Mediterrean & MEDITERRANEAN \\
