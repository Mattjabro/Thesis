...
present in only a handful of samples. Critically, no reference taxonomy of community types
exists: whether recurring clusters reflect genuine ecological units or artefacts of sampling
design cannot be resolved by appeal to any external standard, making unsupervised inference
the only viable approach. The relationship between microbial composition and habitat is
likewise confounded by variation in sequencing depth, land-use history, and geographic
proximity, so high classification accuracy, if achieved, carries real biological meaning.
