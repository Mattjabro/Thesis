...
environmental heterogeneity of the dataset. The results, taken together across the global
topsoil survey and the EMP soil subset, point toward a view of soil microbial communities as
environmentally structured but not deterministically assembled --- community composition
...
assemblages, and that difference overwhelms finer-grained within-terrestrial variation.
...
especially soil pH --- that do not always align neatly with biome boundaries
...
because composition is shaped by a combination of environmental gradients, not by biome
identity alone.
...
entropy ranged from 5.18 to 6.23 across these same clusters. The low diversity of
...
corresponded to terrestrial samples from varied soil environments. This disparity is
consistent with the broader ecological observation that terrestrial soils, and particularly
...
These diversity differences matter beyond description. They confirm that the unsupervised
clustering is not merely grouping samples by overall library size or sequencing noise ---
the clusters capture biologically meaningful differences in community structure that are
reflected in independent metrics computed from the raw count data prior to CLR
transformation.
...
The PCA biplot showed partial separation of MixGGM clusters along the first two components,
with boreal and tropical samples tending to occupy opposite ends of PC1, but with
substantial overlap in between. This pattern is consistent with a continuous environmental
gradient --- most likely driven by temperature and its correlates --- rather than discrete
community types with sharp boundaries.
...
the salinity gradient overwhelms the finer signals that differentiate one terrestrial biome
from another.
...
with clusters overlapping in the two-dimensional embedding. This is not a failure of the
method --- it accurately reflects the underlying data structure, in which terrestrial soil
communities vary more continuously than the discrete cluster labels suggest.
...
signal is present, it is not dominant. Local factors --- pH, moisture, soil texture, land
use history --- introduce enough variation within biomes to blur the compositional
...
biologically interpretable links between microbial composition and habitat. Across the four
models (two datasets $\times$ two label schemes), several genera and higher-level lineages
...
type and soil organic matter quality (Navarrete et al.\ 2015)\cite{navarrete2015}. That
\textit{Ellin516} carried the strongest signal for biome discrimination in the topsoil
dataset suggests that differences in litter chemistry between biomes --- tropical forests
...
well-drained, circumneutral terrestrial soils.
...
of anoxia are common --- and the well-drained temperate and boreal soils where its niche
is less available.
...
Its importance in the classifier may reflect the variable selective pressures across biomes
--- \textit{Bacillus}-dominated communities may be favored in drier grassland and savanna
...
disturbed soils (Asaf et al.\ 2020)\cite{asaf2020}. The high predictive value of these
taxa in the EMP models reflects the dataset's inclusion of agricultural and urban soils,
environments where anthropogenic disturbance selects for different functional guilds than
...
biological system. Soil microbial communities across terrestrial biomes share a large core
of cosmopolitan genera, and it is only shifts in relative abundance --- not the complete
replacement of one community by another --- that distinguish tropical from boreal soils.
...
provides taxonomic information only and cannot directly resolve functional potential. Two
...
the global topsoil data and Deblur for the EMP soil subset --- introducing a potential
source of pipeline-specific bias in the inferred amplicon sequence variants (ASVs). We
mitigated this by collapsing both datasets to genus level and applying the same CLR
...
small (193 samples after filtering). Small per-biome sample sizes --- a single Arctic
tundra sample, for example --- limit the statistical power to characterize rare biome types
and inflate the variance of per-cluster diversity estimates.
