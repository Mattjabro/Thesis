...
Soil harbors the most diverse microbial communities on Earth, yet whether predictable community types exist across biomes and whether composition encodes habitat of origin remain open questions at global scale. This thesis addresses both questions by applying unsupervised clustering and supervised classification to centered log-ratio (CLR)-transformed 16S ribosomal RNA (rRNA) amplicon profiles from two independent datasets: the global topsoil survey of Bahram et al.\ (2018; 193 samples, 165 genus-level lineages after filtering) and the Earth Microbiome Project (EMP) soil subset of Thompson et al.\ (2017; 2,209 samples, 1,533 lineages). A mixture of Gaussian Graphical Models (MixGGM) framework identified 19 non-empty clusters in the topsoil dataset and 10 in the EMP dataset without access to environmental metadata; cluster assignments were significantly associated with biome type, geographic location, and physicochemical variables in both cases. Random forest classifiers trained on the same feature matrices achieved 61.8\% test-set accuracy under grouped biome labels for the topsoil data and 98.2\% for the EMP data. The performance gap reflects the underlying biology: major habitat boundaries (marine vs.\ terrestrial vs.\ freshwater) produce strong compositional contrasts, while within-terrestrial biome differences are subtler and driven by overlapping environmental gradients rather than discrete community turnover. The most predictive taxa --- including \textit{Ellin516}, \textit{Candidatus Udaeobacter}, \textit{Anaeromyxobacter}, \textit{Bacillus}, and \textit{Paenibacillus} --- have known ecological associations consistent with the biome categories they discriminated. Taken together, these results demonstrate that genus-level 16S rRNA profiles carry sufficient information to recover recurring community types and predict habitat of origin, with predictive power scaling with the breadth of environmental contrast in the dataset.
...
number of samples that can be sequenced. 16S ribosomal RNA (rRNA) amplicon sequencing targets a phylogenetically conserved marker
gene present in all bacteria and archaea, allowing community-wide profiling at genus-level
resolution.
...
FASTQ files through a Divisive Amplicon Denoising Algorithm 2 (DADA2) pipeline. The second is the Earth Microbiome Project (EMP)
...
What makes these questions non-trivial is the compositional nature of amplicon count data:
...
Claremont McKenna College (Rocky Linux), managed by SLURM\cite{yoo2003}. Compute nodes provide up to
...
The EMP soil subset data used was a
centrally harmonized ASV table denoised with Deblur\cite{amir2017} at a
...
The $t$-SNE projections reinforced these observations nonlinearly. In the EMP dataset,
...
is moderate. This is not a limitation of the analytical methods; it is a property of the
