...
model_fp <- if (length(args) >= 1) args[1] else {
  file.path(project_root, "models", "best_model_emp_k20.RData")
}
...
extract_clusters <- function(obj, sample_ids) {
  ...
  # Heuristic search for common cluster/membership fields.
  if (is.list(obj)) {
    candidates <- c(
...
merged <- merge(cluster_df, meta, by = "X.SampleID", all.x = TRUE)
write.table(
  merged,
...
cat_vars <- c(
  "env_biome", "env_feature", "env_material", "empo_0", "empo_1", "empo_2",
  "empo_3", "country"
)
num_vars <- c(
