...
classifier_fp <- file.path(ref_dir, "silva_nr99_v138.2_toGenus_trainset_filtered.fa")

# Load ASV table
cat(" Loading ASV table...\n")
seqtab.nochim <- readRDS(file.path(intermediate_dir, "seqtab_nochim.rds"))

# Assign taxonomy (up to Genus level) using the filtered classifier
cat(" Running assignTaxonomy() with filtered classifier...\n")
taxa <- assignTaxonomy(seqtab.nochim, classifier_fp, multithread=TRUE)

# Save taxonomy table (with new filenames to avoid overwriting)
saveRDS(taxa, file.path(intermediate_dir, "taxa_filtered_classifier.rds"))
write.csv(taxa, file.path(intermediate_dir, "taxonomy_table_filtered_classifier.csv"))

# -------------------------
# Barplot & Sample-Taxa Matrix at Genus Level (Level 6)
# -------------------------

cat(" Building barplot and taxa matrix at Genus level...\n")
...
write.csv(rel_abund, file.path(intermediate_dir, "sample_taxa_matrix_L6_filtered_classifier.csv"))
...
p <- ggplot(df_melt, aes(x = Sample, y = RelativeAbundance, fill = Genus)) +
  ...
  labs(title = "Genus-level Relative Abundance (Filtered Classifier)", y = "Proportion", x = "Sample")

dir.create(fig_dir, showWarnings = FALSE, recursive = TRUE)
ggsave(file.path(fig_dir, "genus_level_barplot_filtered_classifier.png"), plot = p, width = 12, height = 6)

cat(" Done! Outputs:\n")
cat(" - taxonomy_table_filtered_classifier.csv\n")
cat(" - sample_taxa_matrix_L6_filtered_classifier.csv\n")
cat(" - genus_level_barplot_filtered_classifier.png\n")
