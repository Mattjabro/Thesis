#!/usr/bin/env Rscript
# ============================================================================
# 24_plot_alpha_diversity_by_cluster.R
#
...
cat("============================================================\n")
cat("  Plotting Alpha Diversity by Cluster (Global Soil)\n")
cat("============================================================\n")

# ============================================================================
# Step 1: Load Data
# ============================================================================
timestamp("Loading alpha diversity and cluster assignments...")
...
# Step 2: Join Data
# ============================================================================
timestamp("Joining alpha diversity with clusters...")

# Standardize cluster sample ID column - both files use the same format
if ("X.SampleID" %in% colnames(clusters)) {
  clusters <- clusters %>% rename(SampleID = X.SampleID)
}

cat("  Example alpha diversity ID: ", alpha_div$SampleID[1], "\n")
cat("  Example cluster ID: ", clusters$SampleID[1], "\n")

# Join directly by SampleID - both files have the same format
cluster_alpha <- alpha_div %>%
  inner_join(clusters, by = "SampleID")

cat("  Samples in alpha diversity: ", nrow(alpha_div), "\n")
cat("  Samples in clusters: ", nrow(clusters), "\n")
cat("  Joined data: ", nrow(cluster_alpha), " samples\n")
cat("  Clusters present: ", n_distinct(cluster_alpha$cluster), "\n")

if (nrow(cluster_alpha) == 0) {
  cat("   ERROR: No samples matched!\n")
  cat("  First few alpha div IDs:\n")
  print(head(alpha_div$SampleID, 5))
  cat("  First few cluster IDs:\n")
  print(head(clusters$SampleID, 5))
  stop("No samples matched between alpha diversity and clusters!")
}

# ============================================================================
# Step 4: Write Joined Table
# ============================================================================
timestamp("Writing joined table...")
...
# ============================================================================
timestamp("Computing summary statistics by cluster...")
...
# ============================================================================
# Step 6: Create Boxplot Visualizations
# ============================================================================
timestamp("Creating boxplot visualizations...")
...
# ============================================================================
# Step 7: Create Scatter Plot with Jitter
# ============================================================================
timestamp("Creating detailed scatter plots...")

create_scatter_plot <- function(data, metric_col, metric_name, filename) {
...
create_scatter_plot(cluster_alpha, "adiv_observed_otus",
                    "Observed ASVs", "alpha_div_observed_scatter_by_cluster.pdf")
create_scatter_plot(cluster_alpha, "adiv_chao1",
                    "Chao1 Estimate", "alpha_div_chao1_scatter_by_cluster.pdf")
create_scatter_plot(cluster_alpha, "adiv_shannon",
                    "Shannon Index", "alpha_div_shannon_scatter_by_cluster.pdf")

# ============================================================================
# Step 8: Create Heatmap of Mean Alpha Diversity
# ============================================================================
timestamp("Creating heatmap summary...")
